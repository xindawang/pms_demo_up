$(document).ready(function(){
	...
	$("#viewplan").click(function(){
		...
		if(showpdf){
			...
			var url = '/ils/zone/downMajorYearPlan/' + new Date().getFullYear() + "/sys;token=" + $.cookie("token");
...
function fileNameValueTo(el, value) {
	...
	el.href = '/ils/zone/downMajorYearPlan/' + new Date().getFullYear() + "/" + grpName + "/sys;token=" + $.cookie("token");
	el.download = value;
}

$(document).ready(function(){
	$.getJSON("../js/zone/plan.js",function(data){				
		render_tree('marjorgrp_content','row_template',data);
	});			
});
