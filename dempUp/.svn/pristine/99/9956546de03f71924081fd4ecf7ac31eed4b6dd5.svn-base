...
(function (window) {
        ...
        while (list.length != 0) {
            for (var i = 0; i < list.length; ++i) {
                ...
                for (var j in columns) {
                    ...
                    if (typeof column.idToName == 'string'){
                    	var td = tr.children('td:last')
                    	var attr = column.idToName.split('.')
                        var data = item
                        for (var k in attr) {
                            if (data != null)
                                data = data[attr[k]]
    ...
    TreeTable.prototype.updateListItem = function (id, item, val) {
        for (var i = 0; i < this.options.list.length; ++i) {
            var row = this.options.list[i];
            if (row.id != id)
                continue;
            row[item] = val;
        }
